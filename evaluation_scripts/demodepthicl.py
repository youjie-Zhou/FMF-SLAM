...
def image_stream(imagedir, depthdir, calib, stride):
    ...
    images_list = images_list[::stride]
    depth_list = depth_list[::stride]
...
##################################################################
        ...
        u_u0_by_fx = (x - cx) / fx
        v_v0_by_fy = (y - cy) / fy
        depths = depths / np.sqrt(u_u0_by_fx * u_u0_by_fx + v_v0_by_fy * v_v0_by_fy + 1)
        depths = depths/1000.0
        # print(depths.mean(),depths.max(),depths.min())
