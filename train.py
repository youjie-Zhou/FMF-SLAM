...
def train(gpu, args):
    ...
    if args.ckpt is not None:
        load_model_state_dict(model, args.ckpt, requires_grad=True)
    # if args.ckpt is not None:
    ...
    while should_keep_training:
        for i_batch, item in enumerate(train_loader):
            ...
            images, poses, disps, intrinsics,depth = [x.to('cuda') for x in item]
            # print(disps.shape,depth.shape)
            ...
            while r < args.restart_prob:
                ...
                poses_est, disps_est, residuals, loss_feature = model(Gs, images, depth,disp0, intrinsics0, 
                    graph, num_steps=args.iters, fixedp=2)

                geo_loss, geo_metrics = losses.geodesic_loss(Ps, poses_est, graph, do_scale=False)
                res_loss, res_metrics = losses.residual_loss(residuals, loss_feature)
                flo_loss, flo_metrics = losses.flow_loss(Ps, disps, poses_est, disps_est, intrinsics, graph)

                loss = args.w1 * geo_loss + args.w2 * res_loss + args.w3 * flo_loss# + 0.1 * loss_feature.sum()
                loss.backward()
            ...
            if total_steps % 5000 == 0 and gpu == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--edges', type=int, default=12)
