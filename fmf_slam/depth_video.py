...
class DepthVideo:
    ...
    def correctloop(self, ii, jj):
        """ project points from ii -> jj """
        Gs = lietorch.SE3(self.poses[None])
        updated_Gs = Gs
        updated_Gs[-1] = Gs[0]
        for i in range(N-1, N-51, -1):
            # 计算从位姿i-1到位姿i的相对位姿Gij
            Gij = Gs[i] * Gs[i-1].inv()
            # 根据相对位姿Gij更新位姿i，使其相对于poses[0]
            # 这里假设poses[0]已经表示了某种“基准”位姿，而你想将Gij相对于这个基准进行更新
            updated_Gs[i-1] = Gij * updated_Gs[i]


        # Gs = lietorch.SE3(self.poses[None])
        # poses[:,jj] * poses[:,ii].inv()
        # coords, valid_mask = \
        #     pops.projective_transform(Gs, self.disps[None], self.intrinsics[None], ii, jj)

        return updated_Gs

    def reprojects(self, ii, jj):
        """ project points from ii -> jj """
        ii, jj = DepthVideo.format_indicies(ii, jj)
        self.poses[-1] = self.poses[0]
        self.poses[-2] = self.poses[1]
        self.poses[-3] = self.poses[2]
        self.poses[-4] = self.poses[3]
        Gs = lietorch.SE3(self.poses[None])
