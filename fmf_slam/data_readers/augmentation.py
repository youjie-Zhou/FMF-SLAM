...
class RGBDAugmentor:
    ...
    def spatial_transform(self, images, depths, poses, intrinsics,depthsss):
        """ cropping and resizing """
        ht, wd = images.shape[2:]
        # print(depthsss.shape)
        ...
        # depthsss = depthsss.unsqueeze(dim=1)
        ...
        depthsss = F.interpolate(depthsss, scale_factor=scale, recompute_scale_factor=True)
        ...
        # print('aug0',images.shape,depthsss.shape)
        ...
        depthsss = depthsss[:, :, y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        depths = depths.squeeze(dim=1)
        depthsss = depthsss.squeeze(dim=1)
        # print('aug',self.crop_size[0],depthsss.shape,images.shape)
        return images, poses, depths, intrinsics,depthsss
    ...
    def __call__(self, images, poses, depths, intrinsics,depthsss):
        images = self.color_transform(images)
        # depthsss = self.color_transform(depthsss)
        return self.spatial_transform(images, depths, poses, intrinsics,depthsss)
